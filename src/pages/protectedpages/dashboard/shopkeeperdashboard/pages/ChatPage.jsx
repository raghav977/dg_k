...
import { fetchConnectedCustomers } from "../../../../../api/Dashboard";

const ShopkeeperChatPage = () => {
  ...
  const { data: connectedData } = useQuery({
    queryKey: ["connectedCustomersForChat"],
    queryFn: () => fetchConnectedCustomers(),
  });

  const connectedCustomers = useMemo(() => {
    if (!connectedData) return [];
    return connectedData.customers || connectedData.results || [];
  }, [connectedData]);
  ...
  const createThreadMutation = useMutation({
    mutationFn: (customerId) => createChatThread({ customer_id: customerId }),
    onSuccess: (thread) => {
  ...
  const handleCreateThread = (e) => {
    ...
    createThreadMutation.mutate(selectedCustomerId);
  };
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-4">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Chat directly with connected customers.</p>
        </div>
        <button
                ...
                {threads.map((thread) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-gray-900">{thread.customer_name}</div>
                      <div className="text-xs text-gray-500">{thread.business_name}</div>
                      {thread.last_message && (
          ...
          {!selectedThread ? (
            ...
            <>
              <div className="p-4 border-b">
                <h3 className="text-base font-semibold text-gray-800">{selectedThread.customer_name}</h3>
                <p className="text-xs text-gray-500">{selectedThread.business_name}</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 space-y-4">
            ...
            <form onSubmit={handleCreateThread} className="space-y-4">
                ...
                >
                  ...
                  {connectedCustomers.map((customer) => (
                    <option key={customer.id} value={customer.id}>
                      {customer.name || customer.customer_name || customer.user?.email}
                    </option>
                  ))}
                </select>
              </label>
              <div className="flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  disabled={createThreadMutation.isLoading || !selectedCustomerId}
...
export default ShopkeeperChatPage;
