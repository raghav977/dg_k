...
const Spinner = ({ size = 16, color = "#fff" }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
    style={{ animation: "ap-spin 0.8s linear infinite", flexShrink: 0 }}>
...
const AddProduct = ({ onClose, onAdded, onLoading }) => {
  ...
  const addProductMutation = useMutation({
    ...
    onError: (err) => console.error("Add product error", err),
  });
  ...
  const isPending = addProductMutation.isPending || addProductMutation.isLoading;
    ...
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      ...
      <div
        ...
        className={`relative w-full rounded-2xl border-2 overflow-hidden transition
          ${preview ? "border-transparent cursor-default" : "border-dashed border-slate-200 hover:border-green-300 cursor-pointer bg-slate-50 hover:bg-green-50/40"}`}
        ...
        {preview ? (
          <>
            ...
            <div className="absolute bottom-2 left-2">
              <span className="text-xs bg-black/50 text-white px-2 py-0.5 rounded-full">{form.image?.name}</span>
            </div>
          </>
        ) : (
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button type="submit" disabled={isPending}
          ...
          {isPending ? <><Spinner /> Saving…</> : "Add Product"}
        </button>
      </div>
    </form>
  );
